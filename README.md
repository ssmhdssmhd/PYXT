...
>
> **版本:** v0.0.8 | **更新日期:** 2026-09-21 | **状态:** ✅ 可运行
...
- 🔌 **解析源可扩展**：内置 `jx.xmflv.cc` 等解析接口，可通过环境变量 `PYXT_PARSE_APIS` 增删（逗号分隔），无需重新编译
- ✅ **直链可播校验**：对候选地址做真实可播验证，只返回 TVBox 等播放器能直接播放的 m3u8/mp4，绝不把 HTML 页当播放源
- 🧭 **无头浏览器兜底**：对加密/JS 播放器源，自动拉起无头浏览器让页面 JS 真实执行，拦截网络请求捕获真实 m3u8（尽力而为）
- ⚡ **浏览器复用提速**：Chromium 进程启动一次后持续复用，空闲自动回收，不再"每个请求都拉起+关闭"浏览器
- 📁 **浏览器资源包外置**：浏览器/引擎部署到 `browser/` 目录即（如 LLQ 引擎包），更新浏览器只换目录、更新程序只换程序，两者分离
...
├── parser.go            # 视频解析核心逻辑（含解析源列表）
├── direct.go            # 直链解析核心：可播校验 + master 多码率选流
├── direct_test.go       # 直链解析单元测试（HTML 提取 / master 选流 / 介质判定）
├── headless.go          # 无头浏览器兜底层：浏览器复用 + 外置浏览器目录 + 网络拦截捕获 m3u8
├── headless_manual_test.go # 无头兜底层手动冒烟测试（需 PYXT_HEADLESS_TEST=1）
├── browser/             # 浏览器资源包目录（可选，用户部署，程序自动识别的可执行浏览器）
│   └── ...              #   放入 chrome/chromium/msedge 可执行文件或其一层的子目录即可
├── llqplayer/           # LLQPlayer 引擎资源目录（用户部署式资源包范例，可直接替换更新）
...
| 变量 | 默认 | 说明 |
|------|------|------|
| `PORT` | `5000` | HTTP 监听端口，被占用时自动 +1 重试 |
| `PYXT_PARSE_APIS` | 内置 5 个 | 第三方解析接口列表，逗号分隔，可增删（顺序即优先级；也用于无头浏览器兜底遍历） |
| `PYXT_HEADLESS` | `1`(启用) | 设为 `0` 关闭无头浏览器兜底 |
| `BROWSER_DIR` | `./browser` | 用户自部署的浏览器资源包目录，程序优先从这里定位浏览器 |
| `PYXT_BROWSER_IDLE` | `180` | 浏览器空闲回收秒数（回收后下次调用自动重新拉起） |

> **浏览器与程序分离部署**：把 Chromium/Chrome/Edge 可执行文件放进 `browser/` 目录即可，程序会自动识别。
> 浏览器更新时只替换该目录，程序更新时只替换程序文件，互不影响。未提供浏览器时才回退到自动下载。

## 🔧 技术说明

- 解析逻辑与原 Python 版 `video_parser_api.py` 保持**完全一致**，包括多级解析策略、标题提取选择器、URL 校验规则
- 解析源列表可扩展：内置 `jx.xmflv.cc` 等，也可用 `PYXT_PARSE_APIS` 覆盖；无头浏览器兜底同样按该顺序遍历
- 无头浏览器采用**进程复用**（启动一次持续使用，空闲自动回收），定位优先级：`browser/` 目录 → `CHROME_BIN` → 系统安装路径 → 自动下载
...
- 🔌 **新增解析源** `https://jx.xmflv.cc/?url=`：加入默认解析源列表（置于首位），同时参与 HTTP 直链提取与无头浏览器兜底，解析出的播放地址随原 JSON **同步返回**（接口本身即为同步阻塞）
- 🔧 **解析源可扩展**：新增环境变量 `PYXT_PARSE_APIS`（逗号分隔），覆盖默认解析源列表，后续增删/调整解析源无需重新编译
- ⚡ **浏览器复用提速**：重构 `headless.go`，无头浏览器 Chromium **进程启动一次后持续复用**，页面级/会话级独立捕获并串行化无头解析，不再"每个请求拉起+关闭"浏览器，显著降低重复解析耗时
- 📁 **浏览器资源包外置**：新增 `browser/` 目录支持（`BROWSER_DIR` 可改）。用户自行部署 Chromium/Chrome/Edge 到该目录（含 `llqplayer/` 这类引擎包），程序优先从该目录定位；浏览器更新只换目录、程序更新只换程序，两者彻底分离
- 🕐 **空闲自动回收**：新增 `PYXT_BROWSER_IDLE`（默认 180 秒），浏览器空闲超时自动关闭释放内存，下次调用自动重新拉起
- 🧪 全部单元测试通过（含 race 检测），`go build` / `go vet` 无告警
